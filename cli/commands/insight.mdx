...
# Get critical issues
cl insight --json | jq '.data[] | select(.severity == "critical")'
```
...
cl insight --json | jq '.data[] | select(.status == "open")'

# Get closed insights
cl insight --json | jq '.data[] | select(.status == "closed")'
```
...
INSIGHTS=$(cl insight --json | jq '.data[] | select(.severity == "high" or .severity == "critical") | .[0:3]')
...
INSIGHT_TITLE=$(cl insight --json | jq -r '.data[0].title')

# Bad - parsing text output
INSIGHT_TITLE=$(cl insight | grep "Title:" | cut -d' ' -f2)
```
