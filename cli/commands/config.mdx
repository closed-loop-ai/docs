...
<ParamField path="--api-key" type="string">
  Your ClosedLoop AI API key. Get it from your dashboard at https://closedloop.sh.
</ParamField>
...
if cl feedback --json > /dev/null 2>&1; then
  ...
  echo "Get a new key from: https://closedloop.sh"
  exit 1
fi
```
...
if [ -n "$CLOSEDLOOP_API_KEY" ]; then
  ...
  echo "❌ CLOSEDLOOP_API_KEY environment variable not set"
  exit 1
fi
