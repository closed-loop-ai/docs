...
while true; do
  ...
  if [ "$COMPLETED_COUNT" -gt 0 ]; then
    echo "✅ New insights available! Run 'cl feedback' to view them."
  fi
...
TODAY_INSIGHTS=$(cl feedback --json | jq --arg today "$TODAY" '
...
WEEKLY_INSIGHTS=$(cl feedback --json | jq --arg start "$START_DATE" --arg end "$END_DATE" '
...
CUSTOMER_INSIGHTS=$(cl feedback --json | jq --arg customer "$CUSTOMER_ID" '
