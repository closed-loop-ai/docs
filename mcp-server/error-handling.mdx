...
**Cause**: The requested feedback item doesn't exist or you don't have access to it.

**Solution**:
- Verify the feedback ID is correct
- Check if the feedback belongs to your team
...
**Cause**: No feedback matches your criteria.

**Solution**:
- Expand your date range
- Check your filtering parameters
- Verify you have feedback data in the system
...
const result = await mcpClient.callTool('list_feedbacks', { limit: 10 });
if (result.error) {
...
function validateFeedbackId(feedbackId) {
...
try {
  const result = await mcpClient.callTool('get_feedback_detail', { feedback_id });
  return result;
} catch (error) {
...
3. **Data Access Issues**
   ...
   - Check if feedback exists in your team's data
