...
```json
{
  "error": "Invalid arguments: \"feedback_id\" is required"
}
```
...
1. **Team Isolation**: All data access is scoped to the team that owns the API key
2. **API Key Security**: Store API keys securely and rotate them regularly
3. **Data Privacy**: Only access data you have permission to view
4. **Audit Logging**: All tool usage is logged for security monitoring
...
- Validate feedback IDs before calling `get_feedback_detail`
