...
const response = await mcpClient.callTool('list_feedbacks', { limit: 10 });
...
async function getCachedFeedbacks(dateFrom, dateTo) {
  ...
  const result = await mcpClient.callTool('list_feedbacks', {
    date_from: dateFrom,
    date_to: dateTo
  });
...
async function getAllFeedbacks(dateFrom, dateTo) {
  const allFeedbacks = [];
  let page = 1;
  ...
  while (true) {
    ...
    allFeedbacks.push(...result.data.feedbacks);
...
async function getMultipleFeedbackDetails(feedbackIds) {
  const results = await Promise.allSettled(
    feedbackIds.map(id => 
      mcpClient.callTool('get_feedback_detail', { feedback_id: id })
    )
  );
